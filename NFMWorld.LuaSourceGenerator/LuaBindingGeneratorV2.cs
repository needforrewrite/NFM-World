...
public class LuaBindingGeneratorV2
{
    ...
    {
        // With the ILuaUserData auto-register pattern, we no longer need
        // an explicit Initialize file. Types self-register via static Metatable.
        // Emit an empty marker file for backward compatibility.
        var fileName = $"LuaBindings.Initialize.g.cs";
        File.WriteAllText(Path.Combine(outputPath, fileName), 
            "// <auto-generated />\n// Types auto-register via ILuaUserData.Metatable. No explicit Initialize needed.\n");
    }

    private void GenerateTypeBindings(LuaVisibleType type, DiscoveredKind kind, string outputPath)
    {
        // Skip types that cannot implement ILuaUserData:
        // - Array types (use StructUserData<T[]> at call sites)
        // - Sealed BCL types from System.* assemblies
        if (type.IsArray)
        {
            Console.WriteLine($"Skipping array type (uses StructUserData): {type.Type.GetFullTypeName()}");
            return;
        }
        if (type.Type.IsSealed && type.Type.FullName?.StartsWith("System.") == true)
        {
            Console.WriteLine($"Skipping sealed BCL type (uses StructUserData): {type.Type.GetFullTypeName()}");
            return;
        }
    ...
    {
        // With ILuaUserData pattern, base infrastructure is provided by Lua-CSharp itself.
        // No need for a generated LuaBindings.Base.g.cs with PushValue/ToObject/etc.
        var fileName = $"LuaBindings.Base.g.cs";
        File.WriteAllText(Path.Combine(outputPath, fileName), 
            "// <auto-generated />\n// Base infrastructure provided by Lua-CSharp. No additional base bindings needed.\n");
    }
