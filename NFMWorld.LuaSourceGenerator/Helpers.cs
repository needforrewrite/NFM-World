...
public static class Helpers
{
    ...
    /// <summary>
    /// Check if a type is one of the four NFMW FixedMath types that map directly
    /// to native LuaValue types (no generated wrapper needed).
    /// </summary>
    public static bool IsFixedMathType(Type t)
    {
        return t == typeof(FixedMathSharp.Fixed64)
            || t == typeof(FixedMathSharp.Vector3d)
            || t == typeof(NFMWorldLibrary.FixedMath.f64AngleSingle)
            || t == typeof(NFMWorldLibrary.FixedMath.f64Euler);
    }

    /// <summary>
    /// Check if a type needs StructUserData&lt;T&gt; wrapping.
    /// </summary>
    public static bool NeedsStructUserData(Type t)
    {
        if (t.IsPrimitive || t == typeof(string) || t == typeof(decimal))
            return false;
        if (IsFixedMathType(t))
            return false;
        if (typeof(Lua.ILuaUserData).IsAssignableFrom(t))
            return false;
        if (t.GetCustomAttribute<LuaVisibleAttribute>() != null)
            return false;
        return true;
    }
}