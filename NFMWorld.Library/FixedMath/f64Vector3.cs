...
/// </summary>
[Serializable]
[TypeConverter(typeof(f64Vector3Converter))]
...
public struct f64Vector3 : IEquatable<f64Vector3>
{
	...
	public class f64Vector3Converter : MathTypeConverter
	{
		#region Public Constructor

		public f64Vector3Converter() : base()
		{
		...
		public override object? ConvertFrom(
			...
			{
				string[] v = s.Split(
					(culture ?? CultureInfo.CurrentCulture).TextInfo.ListSeparator.ToCharArray()
				);
				return new Vector3(
		...
		public override object? ConvertTo(
			...
			{
				Vector3 vec = (Vector3) (value ?? new Vector3());
				return string.Join(
					(culture ?? CultureInfo.CurrentCulture).TextInfo.ListSeparator + " ",
					new string[]
					{
		...
		public override object CreateInstance(
			...
			return new Vector3(
				(float) (propertyValues["X"] ?? 0),
				(float) (propertyValues["Y"] ?? 0),
				(float) (propertyValues["Z"] ?? 0)
			);
		}
