// <auto-generated />
// This file is auto-generated by NFMWorld.LuaSourceGenerator.
// ReSharper disable All
#nullable enable

using LuaNET.LuaJIT;
using static LuaNET.LuaJIT.Lua;

namespace nfm_world_library.Lua;

public partial class LuaBindings
{
    // =========== Bindings for ContO (ContO) ===========
    private static void Register_ContO(lua_State L)
    {
        RegisterMetatable<nfm_world_library.mad.ContO>("MT_ContO");

        // Create metatable for instances
        luaL_newmetatable(L, "MT_ContO");

        // __gc metamethod
        lua_pushcfunction(L, KeepAlive(ContO__gc));
        lua_setfield(L, -2, "__gc");

        // __index metamethod
        lua_pushcfunction(L, KeepAlive(ContO__index));
        lua_setfield(L, -2, "__index");

        // __newindex metamethod
        lua_pushcfunction(L, KeepAlive(ContO__newindex));
        lua_setfield(L, -2, "__newindex");

        // __tostring metamethod
        lua_pushcfunction(L, KeepAlive(ContO__tostring));
        lua_setfield(L, -2, "__tostring");

        lua_pop(L, 1);

        // Create type table for ContO
        lua_newtable(L);

        // Constructor: new()
        lua_pushcfunction(L, KeepAlive(ContO_new));
        lua_setfield(L, -2, "new");

        lua_setglobal(L, "ContO");
    }

    private static int ContO__gc(lua_State L)
    {
        var ptr = lua_touserdata(L, 1);
        if (ptr != 0)
        {
            unsafe
            {
                var id = *(int*)ptr;
                RemoveObject<nfm_world_library.mad.ContO>(id);
            }
        }
        return 0;
    }

    private static int ContO__index(lua_State L)
    {
        var obj = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        var key = lua_tostring(L, 2);
        if (key == null) { lua_pushnil(L); return 1; }

        switch (key)
        {
            case "x":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.X, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).X = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "y":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Y, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Y = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "z":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Z, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Z = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "xz":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Xz, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Xz = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "xy":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Xy, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Xy = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "zy":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Zy, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Zy = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "grat":
                PushValue(L, obj.Grat);
                return 1;
            case "wzy":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Wzy, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Wzy = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "wxz":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Wxz, "MT_fix64", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Wxz = (nfm_world_library.SoftFloat.fix64)value);
                        }
                    }
                }
                return 1;
            case "fcnt":
                PushValue(L, obj.Fcnt);
                return 1;
            case "maxR":
                PushValue(L, obj.MaxR);
                return 1;
            case "keyx":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Keyx, "MT_InlineArray4_Int32", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Keyx = (System.Runtime.CompilerServices.InlineArray4<int>)value);
                        }
                    }
                }
                return 1;
            case "keyz":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Keyz, "MT_InlineArray4_Int32", parentId, static (obj, value) => ((nfm_world_library.mad.ContO)obj).Keyz = (System.Runtime.CompilerServices.InlineArray4<int>)value);
                        }
                    }
                }
                return 1;
            case "damageX":
                lua_pushcfunction(L, KeepAlive(ContO_method_damageX));
                return 1;
            case "damageY":
                lua_pushcfunction(L, KeepAlive(ContO_method_damageY));
                return 1;
            case "damageZ":
                lua_pushcfunction(L, KeepAlive(ContO_method_damageZ));
                return 1;
            case "dust":
                lua_pushcfunction(L, KeepAlive(ContO_method_dust));
                return 1;
            case "spark":
                lua_pushcfunction(L, KeepAlive(ContO_method_spark));
                return 1;
            case "equals":
                lua_pushcfunction(L, KeepAlive(ContO_method_equals));
                return 1;
            case "getHashCode":
                lua_pushcfunction(L, KeepAlive(ContO_method_getHashCode));
                return 1;
            case "toString":
                lua_pushcfunction(L, KeepAlive(ContO_method_toString));
                return 1;
            case "getType":
                lua_pushcfunction(L, KeepAlive(ContO_method_getType));
                return 1;
            default:
                lua_pushnil(L);
                return 1;
        }
    }

    private static int ContO__newindex(lua_State L)
    {
        var obj = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        var key = lua_tostring(L, 2);
        if (key == null) return 0;

        switch (key)
        {
            case "x":
                obj.X = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "y":
                obj.Y = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "z":
                obj.Z = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "xz":
                obj.Xz = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "xy":
                obj.Xy = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "zy":
                obj.Zy = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "wzy":
                obj.Wzy = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "wxz":
                obj.Wxz = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
            case "fcnt":
                obj.Fcnt = ToObject<int>(L, 3)!;
                UpdateStruct(L, 1, obj);
                break;
        }
        return 0;
    }

    private static int ContO__tostring(lua_State L)
    {
        var obj = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);
        lua_pushstring(L, obj.ToString() ?? "");
        return 1;
    }

    private static int ContO_new(lua_State L)
    {
        var argCount = lua_gettop(L);

        if (argCount == 0)
        {
            var obj = new nfm_world_library.mad.ContO();
            PushObject(L, obj, "MT_ContO");
            return 1;
        }

        if (argCount == 1)
        {
            var arg0 = ToObject<nfm_world_library.mad.IInGameCar>(L, 1)!;
            var obj = new nfm_world_library.mad.ContO(arg0);
            PushObject(L, obj, "MT_ContO");
            return 1;
        }

        luaL_error(L, "Invalid arguments for ContO constructor");
        return 0;
    }

    private static int ContO_method_damageX(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 3)
        {
            var arg0 = ToObject<nfm_world_library.mad.CarStats>(L, 2)!;
            var arg1 = ToObject<int>(L, 3)!;
            var arg2 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 4)!;
            self.DamageX(arg0, arg1, arg2);
            UpdateStruct(L, 1, self);
            return 0;
        }

        luaL_error(L, "Invalid arguments for damageX");
        return 0;
    }

    private static int ContO_method_damageY(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 6)
        {
            var arg0 = ToObject<nfm_world_library.mad.CarStats>(L, 2)!;
            var arg1 = ToObject<int>(L, 3)!;
            var arg2 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 4)!;
            var arg3 = ToObject<bool>(L, 5)!;
            var arg4 = ToObject<int>(L, 6)!;
            var arg5 = ToObject<int>(L, 7)!;
            self.DamageY(arg0, arg1, arg2, arg3, arg4, arg5);
            UpdateStruct(L, 1, self);
            return 0;
        }

        luaL_error(L, "Invalid arguments for damageY");
        return 0;
    }

    private static int ContO_method_damageZ(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 3)
        {
            var arg0 = ToObject<nfm_world_library.mad.CarStats>(L, 2)!;
            var arg1 = ToObject<int>(L, 3)!;
            var arg2 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 4)!;
            self.DamageZ(arg0, arg1, arg2);
            UpdateStruct(L, 1, self);
            return 0;
        }

        luaL_error(L, "Invalid arguments for damageZ");
        return 0;
    }

    private static int ContO_method_dust(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 10)
        {
            var arg0 = ToObject<int>(L, 2)!;
            var arg1 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
            var arg2 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 4)!;
            var arg3 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 5)!;
            var arg4 = ToObject<int>(L, 6)!;
            var arg5 = ToObject<int>(L, 7)!;
            var arg6 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 8)!;
            var arg7 = ToObject<int>(L, 9)!;
            var arg8 = ToObject<bool>(L, 10)!;
            var arg9 = ToObject<int>(L, 11)!;
            self.Dust(arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9);
            UpdateStruct(L, 1, self);
            return 0;
        }

        luaL_error(L, "Invalid arguments for dust");
        return 0;
    }

    private static int ContO_method_spark(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 8)
        {
            var arg0 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 2)!;
            var arg1 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 3)!;
            var arg2 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 4)!;
            var arg3 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 5)!;
            var arg4 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 6)!;
            var arg5 = ToObject<nfm_world_library.SoftFloat.fix64>(L, 7)!;
            var arg6 = ToObject<int>(L, 8)!;
            var arg7 = ToObject<int>(L, 9)!;
            self.Spark(arg0, arg1, arg2, arg3, arg4, arg5, arg6, arg7);
            UpdateStruct(L, 1, self);
            return 0;
        }

        luaL_error(L, "Invalid arguments for spark");
        return 0;
    }

    private static int ContO_method_equals(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 1)
        {
            object? arg0;
            if (lua_isnil(L, 2) != 0)
                arg0 = null;
            else
                arg0 = ToObject<object>(L, 2)!;
            var result = self.Equals(arg0);
            UpdateStruct(L, 1, self);
            PushValue(L, result);
            return 1;
        }

        luaL_error(L, "Invalid arguments for equals");
        return 0;
    }

    private static int ContO_method_getHashCode(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 0)
        {
            var result = self.GetHashCode();
            UpdateStruct(L, 1, self);
            PushValue(L, result);
            return 1;
        }

        luaL_error(L, "Invalid arguments for getHashCode");
        return 0;
    }

    private static int ContO_method_toString(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 0)
        {
            var result = self.ToString();
            UpdateStruct(L, 1, self);
            PushValue(L, result);
            return 1;
        }

        luaL_error(L, "Invalid arguments for toString");
        return 0;
    }

    private static int ContO_method_getType(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<nfm_world_library.mad.ContO>(L, 1);

        if (argCount == 0)
        {
            var result = self.GetType();
            UpdateStruct(L, 1, self);
            PushValue(L, result);
            return 1;
        }

        luaL_error(L, "Invalid arguments for getType");
        return 0;
    }

}
