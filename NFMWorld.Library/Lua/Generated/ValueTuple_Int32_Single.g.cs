// <auto-generated />
// This file is auto-generated by NFMWorld.LuaSourceGenerator.
// ReSharper disable All
#nullable enable

using LuaNET.LuaJIT;
using static LuaNET.LuaJIT.Lua;

namespace nfm_world_library.Lua;

public partial class LuaBindings
{
    // =========== Bindings for ValueTuple`2 (ValueTuple_Int32_Single) ===========
    private static void Register_ValueTuple_Int32_Single(lua_State L)
    {
        RegisterMetatable<System.ValueTuple<int, float>>("MT_ValueTuple_Int32_Single");

        // Create metatable for instances
        luaL_newmetatable(L, "MT_ValueTuple_Int32_Single");

        // __gc metamethod
        lua_pushcfunction(L, (ValueTuple_Int32_Single__gc));
        lua_setfield(L, -2, "__gc");

        // __index metamethod
        lua_pushcfunction(L, (ValueTuple_Int32_Single__index));
        lua_setfield(L, -2, "__index");

        // __newindex metamethod
        lua_pushcfunction(L, (ValueTuple_Int32_Single__newindex));
        lua_setfield(L, -2, "__newindex");

        // __tostring metamethod
        lua_pushcfunction(L, (ValueTuple_Int32_Single__tostring));
        lua_setfield(L, -2, "__tostring");

        lua_pop(L, 1);

        // Create type table for ValueTuple_Int32_Single
        lua_newtable(L);

        // Constructor: new()
        lua_pushcfunction(L, (ValueTuple_Int32_Single_new));
        lua_setfield(L, -2, "new");

        lua_setglobal(L, "ValueTuple_Int32_Single");
    }

    private static int ValueTuple_Int32_Single__gc(lua_State L)
    {
        var ptr = lua_touserdata(L, 1);
        if (ptr != 0)
        {
            unsafe
            {
                var id = *(int*)ptr;
                RemoveObject<System.ValueTuple<int, float>>(id);
            }
        }
        return 0;
    }

    private static int ValueTuple_Int32_Single__index(lua_State L)
    {
        var obj = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);

        var key = lua_tostring(L, 2);
        if (key == null) { lua_pushnil(L); return 1; }

        switch (key)
        {
            case "length":
                PushValue(L, ((System.Runtime.CompilerServices.ITuple)obj).Length);
                return 1;
            case "item1":
                PushValue(L, ((System.ValueTuple<int, float>)obj).Item1);
                return 1;
            case "item2":
                PushValue(L, ((System.ValueTuple<int, float>)obj).Item2);
                return 1;
            case "equals":
                lua_pushcfunction(L, (ValueTuple_Int32_Single_method_equals));
                return 1;
            case "compareTo":
                lua_pushcfunction(L, (ValueTuple_Int32_Single_method_compareTo));
                return 1;
            case "getHashCode":
                lua_pushcfunction(L, (ValueTuple_Int32_Single_method_getHashCode));
                return 1;
            case "toString":
                lua_pushcfunction(L, (ValueTuple_Int32_Single_method_toString));
                return 1;
            case "getType":
                lua_pushcfunction(L, (ValueTuple_Int32_Single_method_getType));
                return 1;
            default:
                lua_pushnil(L);
                return 1;
        }
    }

    private static int ValueTuple_Int32_Single__newindex(lua_State L)
    {
        var obj = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);

        var key = lua_tostring(L, 2);
        if (key == null) return 0;

        switch (key)
        {
            case "item1":
                try
                {
                    obj.Item1 = ToObject<int>(L, 3)!;
                }
                catch (System.Exception ex)
                {
                    luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                    return 0;
                }
                UpdateStruct(L, 1, obj);
                break;
            case "item2":
                try
                {
                    obj.Item2 = ToObject<float>(L, 3)!;
                }
                catch (System.Exception ex)
                {
                    luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                    return 0;
                }
                UpdateStruct(L, 1, obj);
                break;
        }
        return 0;
    }

    private static int ValueTuple_Int32_Single__tostring(lua_State L)
    {
        var obj = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);
        lua_pushstring(L, obj.ToString() ?? "");
        return 1;
    }

    private static int ValueTuple_Int32_Single_new(lua_State L)
    {
        var argCount = lua_gettop(L);

        if (argCount == 0)
        {
            var obj = new System.ValueTuple<int, float>();
            PushObject(L, obj, "MT_ValueTuple_Int32_Single");
            return 1;
        }

        if (argCount == 2)
        {
            var arg0 = ToObject<int>(L, 1)!;
            var arg1 = ToObject<float>(L, 2)!;
            try
            {
                var obj = new System.ValueTuple<int, float>(arg0, arg1);
                PushObject(L, obj, "MT_ValueTuple_Int32_Single");
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for ValueTuple`2 constructor");
        return 0;
    }

    private static int ValueTuple_Int32_Single_method_equals(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);

        if (argCount == 1)
        {
            object? arg0;
            if (lua_isnil(L, 2) != 0)
                arg0 = null;
            else
                arg0 = ToObject<object>(L, 2)!;
            try
            {
                var result = ((System.ValueTuple<int, float>)self).Equals(arg0);
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for equals");
        return 0;
    }

    private static int ValueTuple_Int32_Single_method_compareTo(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);

        if (argCount == 1)
        {
            var arg0 = ToObject<System.ValueTuple<int, float>>(L, 2)!;
            try
            {
                var result = ((System.ValueTuple<int, float>)self).CompareTo(arg0);
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for compareTo");
        return 0;
    }

    private static int ValueTuple_Int32_Single_method_getHashCode(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);

        if (argCount == 0)
        {
            try
            {
                var result = ((System.ValueTuple<int, float>)self).GetHashCode();
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for getHashCode");
        return 0;
    }

    private static int ValueTuple_Int32_Single_method_toString(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);

        if (argCount == 0)
        {
            try
            {
                var result = ((System.ValueTuple<int, float>)self).ToString();
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for toString");
        return 0;
    }

    private static int ValueTuple_Int32_Single_method_getType(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<System.ValueTuple<int, float>>(L, 1);

        if (argCount == 0)
        {
            try
            {
                var result = ((System.ValueTuple<int, float>)self).GetType();
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for getType");
        return 0;
    }

}
