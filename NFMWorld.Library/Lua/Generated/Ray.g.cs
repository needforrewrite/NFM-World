// <auto-generated />
// This file is auto-generated by NFMWorld.LuaSourceGenerator.
// ReSharper disable All
#nullable enable

using LuaNET.LuaJIT;
using static LuaNET.LuaJIT.Lua;

namespace nfm_world_library.Lua;

public partial class LuaBindings
{
    // =========== Bindings for Ray (Ray) ===========
    private static void Register_Ray(lua_State L)
    {
        RegisterMetatable<Microsoft.Xna.Framework.Ray>("MT_Ray");

        // Create metatable for instances
        luaL_newmetatable(L, "MT_Ray");

        // __gc metamethod
        lua_pushcfunction(L, (Ray__gc));
        lua_setfield(L, -2, "__gc");

        // __index metamethod
        lua_pushcfunction(L, (Ray__index));
        lua_setfield(L, -2, "__index");

        // __newindex metamethod
        lua_pushcfunction(L, (Ray__newindex));
        lua_setfield(L, -2, "__newindex");

        // Operator: __eq
        lua_pushcfunction(L, (Ray_op_op_Equality));
        lua_setfield(L, -2, "__eq");

        // __tostring metamethod
        lua_pushcfunction(L, (Ray__tostring));
        lua_setfield(L, -2, "__tostring");

        lua_pop(L, 1);

        // Create type table for Ray
        lua_newtable(L);

        // Constructor: new()
        lua_pushcfunction(L, (Ray_new));
        lua_setfield(L, -2, "new");

        lua_setglobal(L, "Ray");
    }

    private static int Ray__gc(lua_State L)
    {
        var ptr = lua_touserdata(L, 1);
        if (ptr != 0)
        {
            unsafe
            {
                var id = *(int*)ptr;
                RemoveObject<Microsoft.Xna.Framework.Ray>(id);
            }
        }
        return 0;
    }

    private static int Ray__index(lua_State L)
    {
        var obj = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);

        var key = lua_tostring(L, 2);
        if (key == null) { lua_pushnil(L); return 1; }

        switch (key)
        {
            case "position":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Position, "MT_Vector3", parentId, static (obj, value) => { System.Diagnostics.Debug.WriteLine($"Attempted to assign value of struct {obj} ({obj.GetType()}) member 'Position' to {value} but Lua only owns a temporary value and there is no way to track it to its parent. Nothing will be set."); });
                        }
                    }
                }
                return 1;
            case "direction":
                {
                    var ptr = lua_touserdata(L, 1);
                    if (ptr != 0)
                    {
                        unsafe
                        {
                            var parentId = *(int*)ptr;
                            PushStructWithParent(L, obj.Direction, "MT_Vector3", parentId, static (obj, value) => { System.Diagnostics.Debug.WriteLine($"Attempted to assign value of struct {obj} ({obj.GetType()}) member 'Direction' to {value} but Lua only owns a temporary value and there is no way to track it to its parent. Nothing will be set."); });
                        }
                    }
                }
                return 1;
            case "equals":
                lua_pushcfunction(L, (Ray_method_equals));
                return 1;
            case "getHashCode":
                lua_pushcfunction(L, (Ray_method_getHashCode));
                return 1;
            case "intersects":
                lua_pushcfunction(L, (Ray_method_intersects));
                return 1;
            case "toString":
                lua_pushcfunction(L, (Ray_method_toString));
                return 1;
            case "getType":
                lua_pushcfunction(L, (Ray_method_getType));
                return 1;
            default:
                lua_pushnil(L);
                return 1;
        }
    }

    private static int Ray__newindex(lua_State L)
    {
        var obj = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);

        var key = lua_tostring(L, 2);
        if (key == null) return 0;

        switch (key)
        {
            case "position":
                try
                {
                    obj.Position = ToObject<Microsoft.Xna.Framework.Vector3>(L, 3)!;
                }
                catch (System.Exception ex)
                {
                    luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                    return 0;
                }
                UpdateStruct(L, 1, obj);
                break;
            case "direction":
                try
                {
                    obj.Direction = ToObject<Microsoft.Xna.Framework.Vector3>(L, 3)!;
                }
                catch (System.Exception ex)
                {
                    luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                    return 0;
                }
                UpdateStruct(L, 1, obj);
                break;
        }
        return 0;
    }

    private static int Ray__tostring(lua_State L)
    {
        var obj = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);
        lua_pushstring(L, obj.ToString() ?? "");
        return 1;
    }

    private static int Ray_op_op_Equality(lua_State L)
    {
        var left = ToObject<Microsoft.Xna.Framework.Ray>(L, 1)!;
        var right = ToObject<Microsoft.Xna.Framework.Ray>(L, 2)!;
        lua_pushboolean(L, left == right ? 1 : 0);
        return 1;
    }

    private static int Ray_new(lua_State L)
    {
        var argCount = lua_gettop(L);

        if (argCount == 0)
        {
            var obj = new Microsoft.Xna.Framework.Ray();
            PushObject(L, obj, "MT_Ray");
            return 1;
        }

        if (argCount == 2)
        {
            var arg0 = ToObject<Microsoft.Xna.Framework.Vector3>(L, 1)!;
            var arg1 = ToObject<Microsoft.Xna.Framework.Vector3>(L, 2)!;
            try
            {
                var obj = new Microsoft.Xna.Framework.Ray(arg0, arg1);
                PushObject(L, obj, "MT_Ray");
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for Ray constructor");
        return 0;
    }

    private static int Ray_method_equals(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);

        if (argCount == 1)
        {
            // Multiple overloads with same argument count - find best match
            int bestScore = -1;
            int bestIndex = -1;

            // Try overload 0: Equals(object)
            {
                int score = 0;
                int score0 = ScoreParameterCompatibility<object>(L, 2);
                if (score0 < 0) goto next0;
                else score += score0;
                if (score > bestScore)
                {
                    bestScore = score;
                    bestIndex = 0;
                }
            }
            next0:

            // Try overload 1: Equals(Microsoft.Xna.Framework.Ray)
            {
                int score = 0;
                int score0 = ScoreParameterCompatibility<Microsoft.Xna.Framework.Ray>(L, 2);
                if (score0 < 0) goto next1;
                else score += score0;
                if (score > bestScore)
                {
                    bestScore = score;
                    bestIndex = 1;
                }
            }
            next1:

            switch (bestIndex)
            {
                case 0:
                    {
                        var arg0 = ToObject<object>(L, 2)!;
                        try
                        {
                            var result = self.Equals(arg0);
                            UpdateStruct(L, 1, self);
                            PushValue(L, result);
                            return 1;
                        }
                        catch (System.Exception ex)
                        {
                            luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                            return 0;
                        }
                    }
                case 1:
                    {
                        var arg0 = ToObject<Microsoft.Xna.Framework.Ray>(L, 2)!;
                        try
                        {
                            var result = self.Equals(arg0);
                            UpdateStruct(L, 1, self);
                            PushValue(L, result);
                            return 1;
                        }
                        catch (System.Exception ex)
                        {
                            luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                            return 0;
                        }
                    }
                default:
                    luaL_error(L, "No compatible overload found for equals");
                    return 0;
            }
        }

        luaL_error(L, "Invalid arguments for equals");
        return 0;
    }

    private static int Ray_method_getHashCode(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);

        if (argCount == 0)
        {
            try
            {
                var result = self.GetHashCode();
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for getHashCode");
        return 0;
    }

    private static int Ray_method_intersects(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);

        if (argCount == 1)
        {
            // Multiple overloads with same argument count - find best match
            int bestScore = -1;
            int bestIndex = -1;

            // Try overload 0: Intersects(Microsoft.Xna.Framework.BoundingBox)
            {
                int score = 0;
                int score0 = ScoreParameterCompatibility<Microsoft.Xna.Framework.BoundingBox>(L, 2);
                if (score0 < 0) goto next0;
                else score += score0;
                if (score > bestScore)
                {
                    bestScore = score;
                    bestIndex = 0;
                }
            }
            next0:

            // Try overload 1: Intersects(Microsoft.Xna.Framework.BoundingSphere)
            {
                int score = 0;
                int score0 = ScoreParameterCompatibility<Microsoft.Xna.Framework.BoundingSphere>(L, 2);
                if (score0 < 0) goto next1;
                else score += score0;
                if (score > bestScore)
                {
                    bestScore = score;
                    bestIndex = 1;
                }
            }
            next1:

            // Try overload 2: Intersects(Microsoft.Xna.Framework.Plane)
            {
                int score = 0;
                int score0 = ScoreParameterCompatibility<Microsoft.Xna.Framework.Plane>(L, 2);
                if (score0 < 0) goto next2;
                else score += score0;
                if (score > bestScore)
                {
                    bestScore = score;
                    bestIndex = 2;
                }
            }
            next2:

            // Try overload 3: Intersects(Microsoft.Xna.Framework.BoundingFrustum)
            {
                int score = 0;
                int score0 = ScoreParameterCompatibility<Microsoft.Xna.Framework.BoundingFrustum>(L, 2);
                if (score0 < 0) goto next3;
                else score += score0;
                if (score > bestScore)
                {
                    bestScore = score;
                    bestIndex = 3;
                }
            }
            next3:

            switch (bestIndex)
            {
                case 0:
                    {
                        var arg0 = ToObject<Microsoft.Xna.Framework.BoundingBox>(L, 2)!;
                        try
                        {
                            var result = self.Intersects(arg0);
                            UpdateStruct(L, 1, self);
                            if (result.HasValue)
                                PushValue(L, result.Value);
                            else
                                lua_pushnil(L);
                            return 1;
                        }
                        catch (System.Exception ex)
                        {
                            luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                            return 0;
                        }
                    }
                case 1:
                    {
                        var arg0 = ToObject<Microsoft.Xna.Framework.BoundingSphere>(L, 2)!;
                        try
                        {
                            var result = self.Intersects(arg0);
                            UpdateStruct(L, 1, self);
                            if (result.HasValue)
                                PushValue(L, result.Value);
                            else
                                lua_pushnil(L);
                            return 1;
                        }
                        catch (System.Exception ex)
                        {
                            luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                            return 0;
                        }
                    }
                case 2:
                    {
                        var arg0 = ToObject<Microsoft.Xna.Framework.Plane>(L, 2)!;
                        try
                        {
                            var result = self.Intersects(arg0);
                            UpdateStruct(L, 1, self);
                            if (result.HasValue)
                                PushValue(L, result.Value);
                            else
                                lua_pushnil(L);
                            return 1;
                        }
                        catch (System.Exception ex)
                        {
                            luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                            return 0;
                        }
                    }
                case 3:
                    {
                        var arg0 = ToObject<Microsoft.Xna.Framework.BoundingFrustum>(L, 2)!;
                        try
                        {
                            var result = self.Intersects(arg0);
                            UpdateStruct(L, 1, self);
                            if (result.HasValue)
                                PushValue(L, result.Value);
                            else
                                lua_pushnil(L);
                            return 1;
                        }
                        catch (System.Exception ex)
                        {
                            luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                            return 0;
                        }
                    }
                default:
                    luaL_error(L, "No compatible overload found for intersects");
                    return 0;
            }
        }

        luaL_error(L, "Invalid arguments for intersects");
        return 0;
    }

    private static int Ray_method_toString(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);

        if (argCount == 0)
        {
            try
            {
                var result = self.ToString();
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for toString");
        return 0;
    }

    private static int Ray_method_getType(lua_State L)
    {
        var argCount = lua_gettop(L) - 1; // First arg is self

        var self = GetStructFromStack<Microsoft.Xna.Framework.Ray>(L, 1);

        if (argCount == 0)
        {
            try
            {
                var result = self.GetType();
                UpdateStruct(L, 1, self);
                PushValue(L, result);
                return 1;
            }
            catch (System.Exception ex)
            {
                luaL_error(L, $"{ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}");
                return 0;
            }
        }

        luaL_error(L, "Invalid arguments for getType");
        return 0;
    }

}
